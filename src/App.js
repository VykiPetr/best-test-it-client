...
function App() {
  ...
  const handleCreateProject = (
    ...
    axios
      .post(`${API_URL}/project/create`, projectCreationData, {
  ...
  const handleProjectAdd = (e) => {
    ...
    if (uploadedAppLogo) {
      ...
      axios
        .post(`${API_URL}/logo-upload`, uploadForm, { withCredentials: true })
        .then((response) => {
      ...
      appLogo =
        "https://www.severnedgevets.co.uk/sites/default/files/styles/medium/public/guides/kitten.png?itok=Wpg9ghjs";
      handleCreateProject(
        appLogo,
        appName.value,
        appDescription.value,
  ...
  const handleProjectEdit = (e, projectId) => {
    ...
    axios
      .post(`${API_URL}/project//edit`, projectEditData, {
        withCredentials: true,
      })
      .then(() => {
        history.push(`/profile/${loggedInUser._id}`);
  ...
  const editProfile = (userImage, aboutMe, mySkills) => {
    ...
    axios
      .post(`${API_URL}/profile/${loggedInUser._id}`, editingProfile, {
        withCredentials: true,
      })
      .then(() => {
        history.push(`/profile/${loggedInUser._id}`);
